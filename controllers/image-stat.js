...
exports.getImageStat = function(req, res)
{
	...
	ImageStat.list(options, function(err, results)
	{
		...
		ImageStat.count().exec(function (err, count)
		{
...
{
        ImageStat.findOne({'user': req.body._pdAccount, 'user_domain': req.body._pdDomain, 'raw_url': req.body.raw_url}, function(err, result)
        {
                if (err)
                {
                        res.status(500);
                ...
                else
                {
                        ...
                        setTimeout(function()
                        {
                                ImageStat.findOne({'user': req.body._pdAccount, 'user_domain': req.body._pdDomain, 'raw_url': req.body.raw_url}, function(err, check)
                                {
                                        if(check == null)
                                        {
                                                console.log("WTF");
